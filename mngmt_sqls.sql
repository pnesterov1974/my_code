...
-- Get information about your cluster nodes and their status (if your database server is in a failover cluster)
...
-- See if buffer pool extension (BPE) is enabled (BPE Enabled)
SELECT [path], 
       [state_description], 
	[current_size_in_kb], 
...
-- Get information about TCP Listener for SQL Server (TCP Listener States)
...
-- Memory Clerk Usage for instance. Look for high value for CACHESTORE_SQLCP (Ad-hoc query plans)
...
------------------------------------------------------------------------------------------------------------------------
-- SQL Server instance info per Database --------------------------------------------------------------------------------
-- //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache. Can take some time on a large database
...
-- Get the average full backup size by month for the current database.
...
------------------------------------------------------------------------------------------------------------------------
-- Queries, Chached plans, Sps          --------------------------------------------------------------------------------
-- //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
-- Top cached queries by Execution Count
...
-- Top Cached SPs By Total Logical Writes. Logical writes relate to both memory and disk I/O pressure 
SELECT TOP(25) p.[name] AS [SP Name], 
...
-- Find missing index warnings for cached plans in the current database. Could take some time on a busy instance
...
WHERE CAST(qp.[query_plan] AS NVARCHAR(MAX)) LIKE N'%MissingIndex%' AND [dbid] = DB_ID()
...
-- Get fragmentation info for all indexes above a certain size in the current database. Could take some time on a very large database.
